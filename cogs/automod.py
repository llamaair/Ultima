...
class automod(commands.Cog): # create a class for our cog that inherits from commands.Cog
    ...
    async def autorule(self, ctx):
        await ctx.guild.create_auto_moderation_rule(name="Anti-spam", event_type=discord.AutoModEventType.message_send, trigger_type=discord.AutoModTriggerType.mention_spam, trigger_metadata=5, actions=[discord.AutoModActionType.block_message()])
